...
import {
  ...
  ISubject,
  IStudentClass,
} from "../redux/RegisterAttendanceData/RegisterAttendanceDataTypes";
...
export const sendRegisterAttendanceInfo = async (
  accessToken: string,
  subject: ISubject,
  classes: IStudentClass[],
  selectedModules: IModule[],
  coordinates: ICoordinates,
  attendanceCodeDuration: IAttendanceCodeDuration,
  ...
  let registerAttendanceDTO: IRegisterAttendanceDTO = {
    ...
    coordinates:
      coordinates.accuracy === 100 && coordinates.latitude === 0
        ? null
        : coordinatesDTO,
    numberOfStudents:
